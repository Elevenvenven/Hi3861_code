...
static void EnvironmentTask(void)
{
    ...
    // IoTGpioInit(FlOWER1_PIN_NAME);
    // hi_io_set_func(FlOWER1_PIN_NAME, HI_IO_FUNC_GPIO_12_GPIO);
    // IoTGpioSetDir(FlOWER1_PIN_NAME, IOT_GPIO_DIR_OUT);

    IoTGpioInit(FlOWER1_PIN_NAME);
    retval = hi_io_set_func(FlOWER1_PIN_NAME, HI_IO_FUNC_GPIO_12_GPIO);
    if (retval != IOT_SUCCESS) {
        printf("IoTGpioInit(12) failed, %0X!\n", retval);
    }
    IoTGpioSetDir(HI_IO_FUNC_GPIO_12_GPIO, IOT_GPIO_DIR_IN);
    ...
    while (measureCnt--) {
       ...
       if (hi_adc_read(HI_ADC_CHANNEL_4, &value,
            ...
            printf("flower1_humidity read error!\n");
            }
        ...
        OledShowString(0, IDX_3, line, 1);
         ...
         ret = snprintf_s(line, sizeof(line), sizeof(line) - 1, "Flower1: %u", (unsigned int)value);
        if (ret < 0) {
            continue;
        }
        OledShowString(0, IDX_4, line, 1);
